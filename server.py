...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "  create_case_from_yaml(yaml_path, machine='ubuntu-latest') — or use a bundle from PATH A/B\n"
        "  → configure_forcings (uses get_glorys_data_script_for_cli; run the output script manually)\n"
        "  → process_forcings\n"
        "  → bundle_case(case_dir, output_dir)\n"
        ...
        "  create_case_from_yaml(yaml_path) — supply a YAML recipe (from `crocodash dump` or handwritten)\n"
        "  → process_forcings\n"
        "  Then build/submit manually or via cesm-runner case_setup/build/submit.\n\n"
        "== NOTES ==\n\n"
        ...
        "  - create_case_from_yaml and fork_case both require configure_forcings + process_forcings\n"
        "    unless the bundle already contains processed forcing files.\n"
